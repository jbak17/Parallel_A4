...
int main(int argc, char **argv)
{
	...
	float* d_xy;
	cudaMalloc(&d_xy, size);

	Mandelbrot* d_Mandel;
	cudaMalloc(&d_Mandel, sizeof(Mandelbrot));

	//copy Mandelbrot metadata to device
	cudaMemcpy(d_Mandel, h_mandel, sizeof(Mandelbrot), cudaMemcpyHostToDevice);

	//figure out blocks
	dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE);

	//figure out threads
	dim3 dimGrid(h_mandel.width / dimBlock.x, h_mandel.height / dimBlock.y);

	//call kernel function
	//MandelbrotFractal<<<dimGrid, dimBlock>>>(d_xy, d_Mandel);

	//get data from kernel to device
	cudaMemcpy(d_xy, h_xy, size, cudaMemcpyDeviceToHost);
